...
export default function Card({ card, cardIndex }: CardProps) {
	...
	const { emitFlip, isGM, settings, emitRedraw, setSelectCardIndex } = useAppContext();
	...
	return (
		<ToolTip content={tooltip || getTooltip()}>
			<TiltCard
				className={`h-[21vh] w-[15vh] max-w-[30vw] relative perspective transition-transform duration-200 z-0 hover:z-10 hover:scale-150 ${isGM ? 'cursor-pointer' : ''} `}
				cardIndex={cardIndex}
			>
				<div
				...
				>
					<div className="absolute inset-0 group backface-hidden">
						...
						<img
							src={getURL(cardBack as TarokkaGameCard, settings)}
							alt="Card Back"
							className={`absolute rounded-lg ${isGM ? 'transition duration-500 group-hover:opacity-0' : ''} ${settings.cardStyle === 'grayscale' ? 'border border-yellow-500/25 group-hover:drop-shadow-[0_0_3px_#ffd700/50]' : ''}`}
						/>
						{isGM && !flipped && (
							<StackTheDeck
